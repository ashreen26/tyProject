...
<div class="toolbar">
    ...
    <form class="flex-fill" method="post">
        <input type="text" name="search" class="form-control" placeholder="<?php echo __('Search for Page');?> .." value="<?php echo $Filter['search'];?>" required>
    </form>
</div>
        ...
        <tbody>
            ...
            <tr class="v-middle text-color">
                ...
                <td class="no-wrap table-link">
                    <a href="<?php echo APP.'/admin/page/'.$Listing['id'];?>">
                        <svg class="icon">
                            <use xlink:href="<?php echo ASSETS.'/img/sprite.svg#edit';?>" />
                        </svg>
                    </a>
                    <a href='<?php echo APP.'/admin/pages?submit={"_ACTION":"delete","id":"'.$Listing['id'].'"}'?>' class="confirm">
                        <svg class="icon">
